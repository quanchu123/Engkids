...
- [ ] 3. Nối trang `src/app/progress/review/page.tsx` với SRS (due-based)
  - [ ] 3.1 Phát hiện đăng nhập (getUser); nếu đã đăng nhập → `getWordsForReview(20)`; rỗng → trạng thái "Hôm nay không có từ cần ôn".
    - _Requirements: 2.1, 2.3_
  - [ ] 3.2 Chấm quality → `await submitReview(id, quality)`, cập nhật stats phiên, sang từ kế; lỗi → toast nhẹ, giữ tiến độ.
    - _Requirements: 3.1, 3.4, 6.2_
  - [ ] 3.3 Khách (chưa đăng nhập) → giữ chế độ cũ (savedWords + updateWordMastery) + banner mời đăng nhập.
    - _Requirements: 6.1_
  - [ ] 3.4 Hiển thị tiến độ "đã ôn / tổng đến hạn"; tái dùng UI flashcard + phát âm hiện có.
    - _Requirements: 2.4_

- [ ] 4. Hiển thị "đến hạn hôm nay" + thống kê trên `src/app/progress/page.tsx`
...
- [ ] 5. Đồng bộ từ đã lưu → vocabulary_items (best-effort)

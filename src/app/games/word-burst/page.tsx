...
function Burst({ x, y, color, onDone }: BurstProps) {
  ...
  return (<div className="fixed pointer-events-none" style={{ left: x, top: y, zIndex: 1000 }}>{particles.map((p, i) =>{
...
function Orb({ word, grad, size, x, y, floatDelay, state, onClick }: OrbProps) {
  ...
  return (<button
      ...
      style={{
        ...
        top:    y - size / 2,
        background: wrong
          ?'radial-gradient(circle at 35% 30%, #ff4444, #aa0000)': correct
          ?'radial-gradient(circle at 35% 30%, #22ff88, #008844)':`radial-gradient(circle at 35% 30%, ${grad[0]}cc, ${grad[1]})`,
        boxShadow: wrong
          ?'0 0 30px #ff444488, inset 0 0 15px rgba(255,255,255,0.15)': correct
          ?'0 0 40px #22ff8844, 0 0 80px #22ff8822, inset 0 0 15px rgba(255,255,255,0.2)': idle
          ?`0 0 20px ${grad[0]}55, inset 0 0 12px rgba(255,255,255,0.12)`:'none',
    ...
    >{/* Specular highlight */}<div className="absolute rounded-full pointer-events-none"
        style={{ width:'40%', height:'25%', top:'15%', left:'22%',
          background:'radial-gradient(ellipse, rgba(255,255,255,0.55) 0%, transparent 80%)'}} /><span className="relative z-10 px-1 text-center leading-tight">{word}</span></button>);
}
...
export default function WordBurstPage() {
  ...
  const handleOrbClick = useCallback((idx: number, e: React.MouseEvent) =>{
    ...
    if (correct) {
      ...
      setTimeout(() =>{
        setOrbStates(prev =>prev.map((s, i) =>roundData.choices[i].en === roundData.correct.en ?'correct': s ==='idle'?'dimmed': s
        ));
  ...
  return (<>{/* ── CSS animation keyframes ── */}<style>{`@keyframes orb-float {
      ...
      >{/* ── Starfield ── */}
        {Array.from({ length: 80 }, (_, i) =>(<div key={i} className="absolute rounded-full pointer-events-none" style={{
        ...
        {/* ── HUD ── */}
        {!gameOver && (<div className="absolute top-0 inset-x-0 z-30 pointer-events-none"
            style={{ background:'linear-gradient(to bottom, rgba(0,0,0,0.75) 0%, transparent 100%)'}}><div className="flex items-center justify-between px-4 pt-3 pb-1">{/* Back */}<div className="pointer-events-auto"><Link href="/games"
                  className="flex items-center gap-1 text-xs font-bold px-3 py-1.5 rounded-lg transition-colors"
                  style={{ color:'#94a3b8', background:'rgba(255,255,255,0.07)', border:'1px solid rgba(255,255,255,0.12)'}}>← Lobby</Link></div>{/* Score + streak */}<div className="flex items-center gap-3">{streak >= 3 && (<span className="text-sm font-black text-amber-400 animate-bounce">x{streak}</span>)}<span className="font-black text-lg" style={{ color:'#fbbf24', textShadow:'0 0 12px #fbbf2488'}}>{score}</span></div>{/* HP as colored orbs */}<div className="flex gap-1.5">{Array(3).fill(0).map((_, i) =>(<div key={i} className="w-5 h-5 rounded-full transition-all duration-300" style={{
                    background: i< hp
                      ?'radial-gradient(circle at 35% 35%, #f87171, #dc2626)':'rgba(255,255,255,0.08)',
                    boxShadow: i< hp ?'0 0 8px #dc262688':'none',
                  }} />))}</div></div>{/* Progress dots + round */}<div className="flex items-center justify-center gap-1.5 py-1">{Array(TOTAL_ROUNDS).fill(0).map((_, i) =>(<div key={i} className="rounded-full transition-all duration-500" style={{
                  width:  i === round ? 14 : 7,
                  height: 7,
        ...
        {/* ── Question word ── */}
        {!gameOver && orbLayout.length >0 && (<div className="absolute left-1/2 -translate-x-1/2 z-20 flex flex-col items-center pointer-events-none"
            style={{ top:'13%'}}><span className="text-white/40 text-xs font-bold uppercase tracking-[0.2em]">Tìm nghĩa tiếng Anh của</span><h1 className="font-black text-4xl sm:text-5xl mt-1"
              style={{
                color:'#fde68a',
                animation:'title-pulse 2.2s ease-in-out infinite',
                letterSpacing:'0.02em',
              }}>{roundData.correct.vi}</h1></div>)}

        {/* ── Orbs ── */}
        {!gameOver && orbLayout.map((orb, idx) =>(
          roundData.choices[idx] && (<Orb
              key={`${round}-${idx}`}
        ...
        {/* ── Burst particles ── */}
        {bursts.map(b =>(<Burst key={b.id} x={b.x} y={b.y} color={b.color} onDone={() =>removeBurst(b.id)} />))}

        {/* ── Controls hint ── */}
        {!gameOver && round === 0 && orbStates.every(s =>s ==='idle') && (<div className="absolute bottom-6 inset-x-0 text-center pointer-events-none"
            style={{ color:'rgba(255,255,255,0.25)', fontSize:'12px'}}>Chạm vào quả cầu có nghĩa Tiếng Anh đúng</div>)}

        {/* ── Win Screen ── */}
        {gameOver ==='win'&& (<div className="absolute inset-0 flex flex-col items-center justify-center gap-6 z-40"
            style={{ background:'radial-gradient(ellipse at center, #0f3310 0%, #000000 100%)'}}>{/* Stars */}<div className="flex gap-3">{Array(3).fill(0).map((_, i) =>(<div key={i} className="text-5xl" style={{
                  animation:`star-in 0.5s ${i * 0.2 + 0.3}s ease-out both`,
                  filter: i< stars ?'drop-shadow(0 0 12px gold)':'grayscale(1) opacity(0.25)',
                }}></div>))}</div><h2 className="font-black text-5xl sm:text-6xl" style={{
              background:'linear-gradient(135deg, #fde68a, #f59e0b, #fbbf24)',
              WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent',
              filter:'drop-shadow(0 0 20px rgba(251,191,36,0.6))',
            }}>XUẤT SẮC!</h2><p className="text-white/60 text-lg">{TOTAL_ROUNDS} câu ·<span className="text-yellow-400 font-black">{score} điểm</span></p><button onClick={handleReplay}
              className="px-10 py-4 rounded-full font-black text-xl text-black transition-transform hover:scale-105 active:scale-95"
              style={{ background:'linear-gradient(135deg, #fde68a, #f59e0b)', boxShadow:'0 0 30px rgba(245,158,11,0.5)'}}>Chơi lại</button><Link href="/games" className="text-white/30 hover:text-white text-sm underline">Quay về lobby</Link></div>)}

        {/* ── Lose Screen ── */}
        {gameOver ==='lose'&& (<div className="absolute inset-0 flex flex-col items-center justify-center gap-6 z-40"
            style={{ background:'radial-gradient(ellipse at center, #1a0000 0%, #000000 100%)'}}><div className="text-7xl"></div><h2 className="font-black text-5xl sm:text-6xl" style={{
              background:'linear-gradient(135deg, #fc8181, #dc2626)',
              WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent',
              filter:'drop-shadow(0 0 20px rgba(220,38,38,0.6))',
            }}>GAME OVER</h2><p className="text-white/60 text-lg">Điểm:<span className="text-orange-400 font-black">{score}</span></p><button onClick={handleReplay}
              className="px-10 py-4 rounded-full font-black text-xl text-white transition-transform hover:scale-105 active:scale-95"
              style={{ background:'linear-gradient(135deg, #dc2626, #7c3aed)', boxShadow:'0 0 30px rgba(220,38,38,0.4)'}}>Thử lại</button><Link href="/games" className="text-white/30 hover:text-white text-sm underline">Quay về lobby</Link></div>)}</div></>);
}

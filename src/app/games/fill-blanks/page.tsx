...
export default function FillBlanksPage() {
  ...
  const renderSentence = (sentence: string) =>{
    ...
    return (<span>{parts[0]}<span className={`inline-block min-w-[80px] border-b-4 mx-1 text-center font-black ${
          selected
            ? isCorrect
              ?'border-green-500 text-green-600':'border-red-500 text-red-600':'border-purple-400 text-purple-500'}`}>{selected ||'______'}</span>{parts[1]}</span>);
  };

  // Start screen
  if (!difficulty) {
    return (<><Header /><main className="min-h-screen bg-gradient-to-b from-purple-50 via-indigo-50 to-blue-50 pb-16"><div className="max-w-2xl mx-auto px-4 pt-10"><Link href="/games" className="text-purple-600 font-bold text-sm hover:underline">← Quay lại</Link><div className="text-center mt-6"><div className="text-6xl mb-4"></div><h1 className="text-3xl font-black text-gray-800 mb-2">Điền từ vào chỗ trống</h1><p className="text-gray-500 mb-8">Chọn từ đúng để hoàn thành câu tiếng Anh</p></div><div className="space-y-3">{(Object.keys(DIFFICULTY_LABELS) as Difficulty[]).map((diff) =>(<button
                  key={diff}
                ...
                ><div className="font-black text-lg">{DIFFICULTY_LABELS[diff]}</div><div className="text-white/80 text-sm mt-1">{QUESTIONS_BY_DIFFICULTY[diff].length} câu hỏi · 10 câu mỗi lượt</div></button>))}</div></div></main></>);
  }

  // Finished screen
  if (finished) {
    const pct = Math.round((score / TOTAL) * 100);
    return (<><Header /><main className="min-h-screen bg-gradient-to-b from-purple-50 via-indigo-50 to-blue-50 flex items-center justify-center pb-16"><div className="text-center px-4"><div className="text-7xl mb-4">{pct >= 80 ?'': pct >= 50 ?'':''}</div><h2 className="text-3xl font-black text-gray-800 mb-2">Hoàn thành!</h2><p className="text-5xl font-black text-purple-600 mb-2">{score}/{TOTAL}</p><p className="text-gray-500 mb-6">{pct}% chính xác</p><div className="flex gap-3 justify-center flex-wrap"><button onClick={() =>startGame(difficulty)} className="bg-purple-600 text-white font-bold px-6 py-3 rounded-xl hover:bg-purple-700 transition">Chơi lại</button><button onClick={() =>setDifficulty(null)} className="bg-gray-200 text-gray-700 font-bold px-6 py-3 rounded-xl hover:bg-gray-300 transition">Đổi độ khó</button><Link href="/games" className="bg-gray-100 text-gray-600 font-bold px-6 py-3 rounded-xl hover:bg-gray-200 transition">Quay lại</Link></div></div></main></>);
  }
  ...
  return (<><Header /><main className="min-h-screen bg-gradient-to-b from-purple-50 via-indigo-50 to-blue-50 pb-16"><div className="max-w-2xl mx-auto px-4 pt-6">{/* Top bar */}<div className="flex items-center justify-between mb-6"><Link href="/games" className="text-purple-600 font-bold text-sm hover:underline">← Quay lại</Link><div className="flex items-center gap-3"><span className="text-sm font-bold text-gray-600">{score}/{TOTAL}</span><span className="bg-purple-100 text-purple-700 text-xs font-bold px-3 py-1 rounded-full">{currentIndex + 1}/{questions.length}</span></div></div>{/* Progress bar */}<div className="w-full bg-purple-100 rounded-full h-3 mb-8"><div
            ...
            /></div>{/* Sentence card */}<div className="bg-white rounded-3xl shadow-xl p-8 mb-6 text-center"><p className="text-2xl sm:text-3xl font-bold text-gray-800 leading-relaxed">{renderSentence(q.sentence)}</p>{showHint && (<p className="text-purple-500 text-sm mt-3 font-semibold">Gợi ý: {q.hint}</p>)}
          ...
          {selected && (<div className="text-center"><p className={`text-lg font-black mb-4 ${isCorrect ?'text-green-600':'text-red-600'}`}>{isCorrect ?'Chính xác!':`Sai rồi! Đáp án đúng: "${q.answer}"`}</p><button
                onClick={handleNext}
              ...
              >{currentIndex + 1 >= questions.length ?'Xem kết quả':'Câu tiếp theo →'}</button></div>)}</div></main></>);
}

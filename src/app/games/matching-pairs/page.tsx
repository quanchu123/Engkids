...
export default function MatchingPairsPage() {
  ...
  const getCardStyle = (card: Card) => {
    ...
    if (selected.includes(card.id)) {
      return card.type === 'en'
        ? 'bg-purple-200 border-purple-500 text-purple-800 scale-105 shadow-xl'
        : 'bg-pink-200 border-pink-500 text-pink-800 scale-105 shadow-xl';
    }
    return card.type === 'en'
      ? 'bg-white border-purple-200 text-gray-800 hover:border-purple-400 hover:bg-purple-50 hover:scale-105 cursor-pointer'
      : 'bg-white border-pink-200 text-gray-800 hover:border-pink-400 hover:bg-pink-50 hover:scale-105 cursor-pointer';
  };
  ...
  if (!difficulty) {
    return (
      <>
        <Header />
        <main className="min-h-screen bg-gradient-to-b from-fuchsia-50 via-pink-50 to-purple-50 pb-16">
          <div className="max-w-2xl mx-auto px-4 pt-10">
            <Link href="/games" className="text-purple-600 font-bold text-sm hover:underline">← Quay lại</Link>
            <div className="text-center mt-6">
              <div className="text-6xl mb-4">🔗</div>
              <h1 className="text-3xl font-black text-gray-800 mb-2">Ghép cặp từ vựng</h1>
              <p className="text-gray-500 mb-8">Nối từ tiếng Anh với nghĩa tiếng Việt tương ứng trước khi hết giờ!</p>
            </div>
            <div className="space-y-3">
              {(Object.keys(DIFFICULTY_CONFIG) as Difficulty[]).map((diff) => (
                ...
                >
                  <div className="font-black text-lg">{DIFFICULTY_CONFIG[diff].label}</div>
                  <div className="text-white/80 text-sm mt-1">
                    {DIFFICULTY_CONFIG[diff].pairs} cặp từ · {DIFFICULTY_CONFIG[diff].time}s
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="max-w-3xl mx-auto px-4 pt-6">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3 text-sm font-bold">
              <span className="bg-white px-3 py-1 rounded-full shadow text-gray-700">🎯 {matchedPairs.length}/{totalPairs}</span>
              <span className="bg-white px-3 py-1 rounded-full shadow text-gray-700">🔁 {moves}</span>
              <span className={`px-3 py-1 rounded-full shadow font-black ${
                timeLeft <= 15 ? 'bg-red-100 text-red-600 animate-pulse' : 'bg-white text-gray-700'
              }`}>
          ...
          {/* Legend */}
          <div className="flex gap-3 justify-center mb-4 text-xs font-bold">
            <span className="bg-purple-100 text-purple-700 px-3 py-1 rounded-full">🇬🇧 Tiếng Anh</span>
            <span className="bg-pink-100 text-pink-700 px-3 py-1 rounded-full">🇻🇳 Tiếng Việt</span>
          </div>
            ...
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-3xl shadow-2xl p-8 text-center max-w-sm w-full">
                ...
                <h2 className="text-2xl font-black text-gray-800 mb-2">
                  {won ? 'Xuất sắc!' : 'Hết giờ!'}
                </h2>
                <p className="text-gray-500 mb-2">
                  {won
                    ? `Ghép đúng tất cả ${totalPairs} cặp chỉ với ${moves} lần!`
                    : `Bạn đã ghép được ${matchedPairs.length}/${totalPairs} cặp.`}
                </p>

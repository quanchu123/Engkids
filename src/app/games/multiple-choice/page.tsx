    ...
    return (
      <>
        ...
        <div className="min-h-screen bg-gradient-to-br from-purple-400 via-purple-300 to-purple-500 flex items-center justify-center p-4">
          <div className="max-w-lg w-full bg-white/95 rounded-3xl shadow-2xl p-8 text-center border-4 border-blue-200">
            <Link href="/games" className="inline-block mb-4 text-purple-600 hover:text-purple-800 font-bold text-sm">
              ← Quay lại
            </Link>
            <div className="text-6xl mb-4">📋</div>
            <h1 className="text-3xl font-black bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent mb-3">
              Trắc Nghiệm Tiếng Anh
            </h1>
            <p className="text-gray-600 mb-8">Chọn đáp án đúng cho mỗi câu hỏi!</p>
              ...
              {Object.entries(DIFFICULTY_LABELS).map(([key, label]) => (
                ...
                >
                  ...
                  <span className="block text-sm opacity-80 mt-1">
                    {key === 'beginner' ? 'Từ vựng cơ bản' : key === 'intermediate' ? 'Từ vựng hàng ngày' : 'Từ vựng nâng cao'}
                  </span>
                </button>
              ))}
            </div>
        ...
        <div className="min-h-screen bg-gradient-to-br from-purple-400 via-purple-300 to-purple-500 flex items-center justify-center p-4">
          <div className="max-w-lg w-full bg-white/95 rounded-3xl shadow-2xl p-8 text-center border-4 border-blue-200">
            ...
            <div className="bg-blue-50 rounded-2xl p-6 mb-6 space-y-2">
              ...
              <p className="text-lg text-purple-600">⏱️ {formatTime(time)}</p>
              <p className="text-lg text-green-600">✅ {score / 10}/{questions.length} câu đúng</p>
            </div>
            <div className="flex flex-wrap gap-3 justify-center">
              <button onClick={() => startGame(difficulty)} className="px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-full font-bold shadow-lg hover:scale-105 transition-transform">
                Chơi lại 🔄
              </button>
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-purple-400 via-purple-300 to-purple-500 p-4">
        <div className="max-w-2xl mx-auto">
          {/* Game header */}
          <div className="bg-gradient-to-r from-blue-500 to-purple-600 rounded-2xl shadow-xl p-5 mb-5 text-white">
            <div className="flex justify-between items-center mb-3">
              <span className="text-lg font-bold">Câu {current + 1}/{questions.length}</span>
              <span className="text-blue-100">⏱️ {formatTime(time)}</span>
          ...
          {/* Question */}
          <div className="bg-white/95 rounded-2xl shadow-xl p-7 border-2 border-blue-200">
            <h2 className="text-2xl font-black text-gray-800 mb-7 text-center">
              {question.question}
            </h2>
            ...
            {feedback && (
              <div className={`text-center mb-5 p-5 rounded-2xl shadow-lg ${
                ...
                <p className="text-xl font-bold mb-1">
                  {feedback === 'correct' ? 'Chính xác! 🎉' : `Sai rồi! ❌ Đáp án: ${question.answer}`}
                </p>

...
import { addStory, getAllTopics } from '@/data/stories';
...
export default function NewStoryPage() {
  ...
  const [panels, setPanels] = useState<PanelForm[]>([
    { id: generateId(), image: '', sentence_en: '', sentence_vi: '' }
  ]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Build story object
      const storyId = titleEn.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') + '-' + generateId();

...
export function useToast() {
  const showToast = useCallback((message: string, options: ToastOptions = {}) => {
    ...
    if (typeof window !== 'undefined') {
      ...
      toast.className = `fixed bottom-4 right-4 z-50 max-w-sm p-4 rounded-lg shadow-lg transition-all transform translate-y-0 opacity-100 ${getToastColor(type)}`;
      toast.setAttribute('role', 'alert');
      toast.setAttribute('aria-live', 'polite');

      // Build content with DOM API to prevent XSS
      ...
      iconWrapper.innerHTML = getToastIcon(type); // Safe: static hardcoded SVG
      ...
      closeBtn.className = 'flex-shrink-0 text-current opacity-70 hover:opacity-100';
  ...
  const error = useCallback((message: string, options?: Omit<ToastOptions, 'type'>) => {
    showToast(message, { ...options, type: 'error', duration: options?.duration || 8000 });
  }, [showToast]);

...
export async function getNextLearningAction(profileId: string | null): Promise<{ action: NextLearningAction; learnerState: LearnerCurriculumState | null; dueWords: number }> {
  ...
  if (learnerState?.needsLevelSelection || learnerState?.placementDone === false) {
    return {
      ...
      action: {
        kind: 'placement',
        title: 'Kiểm tra xếp trình độ nhẹ nhàng',
        description: 'Bắt đầu bằng một bài kiểm tra ngắn để Engkids xác định đúng cấp độ CEFR.',
        href: '/learn/placement',
        priority: 100,
        reason: 'Chưa có kết quả xếp trình độ được lưu.',
        stageId,
        learningMode,
        weakSkill,
      },
  ...
  if (dueWords >= 8 || weakSkill) {
    return {
      ...
      action: {
        kind: 'review',
        title: weakSkill ? `Củng cố ${weakSkill}` : 'Ôn các từ đến hạn',
        description: weakSkill ? 'Ôn nhanh theo từng bước trước khi học nội dung mới.' : `Ôn ${dueWords} từ đến hạn bằng luyện tập SRS.`,
        href: '/progress/review',
        priority: 90,
        reason: weakSkill ? 'Có một kỹ năng đang dưới ngưỡng thành thạo 60%.' : 'Số từ đến hạn SRS đủ cao nên nên ôn trước.',
        stageId,
        learningMode,
        weakSkill,
      },
  ...
  if (checkpointDue) {
    return {
      ...
      action: {
        kind: 'checkpoint',
        title: 'Kiểm tra mốc nhẹ nhàng',
        description: 'Một bài kiểm tra ngắn sẽ cập nhật mức độ thành thạo và gợi ý bước tiếp theo.',
        href: `/learn/checkpoint?stage=${encodeURIComponent(stageId)}`,
        priority: 80,
        reason: 'Đã đến hạn kiểm tra mốc đã lưu.',
        stageId,
        learningMode,
        weakSkill,
      },
  ...
  if (nextLesson) {
    return {
      ...
      action: {
        ...
        description: learningMode === 'kid' ? 'Một bài học ngắn, vui nhộn với luyện tập và phần thưởng nhỏ.' : 'Một bài học tập trung cho thiếu niên với phần luyện đầu ra.',
        href: `/learn/lessons/${encodeURIComponent(nextLesson.id)}`,
        priority: 70,
        reason: 'Bài học chưa hoàn thành tiếp theo trong cấp độ CEFR hiện tại.',
        stageId,
        learningMode,
        weakSkill,
      },
  ...
  return {
    ...
    action: {
      ...
      description: 'Giữ nhịp học bằng một trò chơi từ vựng ngắn.',
      href: `/games?stage=${encodeURIComponent(stageId)}`,
      priority: 40,
      reason: 'Không tìm thấy bài học chưa hoàn thành ở cấp độ này.',
      stageId,
      learningMode,
      weakSkill,
    },
